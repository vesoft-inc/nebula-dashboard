...
export const LINUX = {
  ...
  disk_size: 'node_filesystem_size_bytes{mountpoint="/",fstype!="rootfs"}',
  network_in_rate: 'sum by(instance)(irate(node_network_receive_bytes_total{device=~"(eth|en)[a-z0-9]*"}[1m]))',
  network_out_rate: 'sum by(instance)(irate(node_network_transmit_bytes_total{device=~"(eth|en)[a-z0-9]*"}[1m]))',
  network_in_errs: 'sum by(instance)(irate(node_network_receive_errs_total{device=~"(eth|en)[a-z0-9]*"}[1m]))',
  network_out_errs: 'sum by(instance)(irate(node_network_transmit_errs_total{device=~"(eth|en)[a-z0-9]*"}[1m]))',
  network_in_packets: 'sum by(instance)(irate(node_network_receive_packets_total{device=~"(eth|en)[a-z0-9]*"}[1m]))',
  network_out_packets: 'sum by(instance)(irate(node_network_transmit_packets_total{device=~"(eth|en)[a-z0-9]*"}[1m]))',
};
